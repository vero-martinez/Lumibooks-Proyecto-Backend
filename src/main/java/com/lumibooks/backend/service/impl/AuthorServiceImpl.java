 ...
 * Se encarga de manejar la lógica de negocio relacionada con los autores, tanto para la administración
 * (como creación, actualización, listado con filtros y activación/desactivación) como para la visualización pública
 * (listado público y detalles de autor). Se asegura de validar la unicidad del nombre y apellido del autor, y 
 * de manejar correctamente el estado activo/inactivo para la visibilidad pública.
 */
@Service
...
public class AuthorServiceImpl implements AuthorService {
     ...
     * El resultado se mapea a AuthorSummaryResponse para mostrar solo la información necesaria en listados.
     * @param name Filtro opcional para buscar por nombre o apellido (case-insensitive).
     * @param isActive Filtro opcional para buscar solo autores activos o inactivos.
     * @param pageable Información de paginación y ordenamiento.
     * @return Página de AuthorSummaryResponse con los autores que cumplen los filtros.
     */
    @Override
     ...
     * Busca el autor en la base de datos y el resultado se mapea a AuthorAdminResponse.
     ...
     * El autor se crea con estado activo por defecto y el resultado se mapea a AuthorAdminResponse.
     * @param authorRequest datos del autor a crear.
     * @return detalles del autor creado para la vista de administración.
     */
    @Override
    @Transactional
    public AuthorAdminResponse createAuthor(AuthorRequest authorRequest) {
        ...
        Author author = Author.builder()
                ...
                .profileImageUrl(authorRequest.getProfileImageUrl())
                .isActive(true)
                .build();
    ...
    @Override
    @Transactional
    public AuthorAdminResponse updateAuthor(Long id, AuthorRequest authorRequest) {
        Author author = authorRepository.findById(id)
                .orElseThrow(() -> new ResourceNotFoundException("Autor no encontrado"));
        ...
        if (authorRepository.existsByFirstNameIgnoreCaseAndLastNameIgnoreCase(
                ...
                && !(author.getFirstName().equalsIgnoreCase(authorRequest.getFirstName())
                        && author.getLastName().equalsIgnoreCase(authorRequest.getLastName()))) {
            throw new BadRequestException("Ya existe un autor con ese nombre y apellido");
        }
     ...
     * Este cambio permite controlar la visibilidad pública del autor sin eliminar su registro.
     * @param id identificador del autor.
     */
    @Override
    @Transactional
     ...
     * El resultado se mapea a AuthorPublicResponse para mostrar únicamente la información pública del autor.
     * @param name filtro opcional para buscar por nombre o apellido (case-insensitive).
     * @param pageable información de paginación y ordenamiento.
     * @return página de AuthorPublicResponse con los autores activos que cumplen el filtro.
     */
    @Override
    @Transactional(readOnly = true)
     ...
     * El resultado se mapea a AuthorPublicResponse para exponer únicamente la información pública del autor.
     * @param id identificador del autor.
